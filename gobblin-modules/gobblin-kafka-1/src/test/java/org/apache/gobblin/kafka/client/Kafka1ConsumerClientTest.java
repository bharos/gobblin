...
public class Kafka1ConsumerClientTest {
  ...
  public void testConsume()
    ...
    ConsumerRecord<String, String> record0 =
        new ConsumerRecord<>("test_topic", 0, 0L, 10L, TimestampType.CREATE_TIME, 0L, 3, 6, "key", "value0");
    ConsumerRecord<String, String> record1 =
        new ConsumerRecord<>("test_topic", 0, 1L, 11L, TimestampType.LOG_APPEND_TIME, 1L, 3, 6, "key", "value1");
    ConsumerRecord<String, String> record2 =
        new ConsumerRecord<>("test_topic", 0, 2L, 12L, TimestampType.LOG_APPEND_TIME, 2L, 3, 6, "key", "value2");
    ...
    try (Kafka1ConsumerClient<String, String> kafka1Client = new Kafka1ConsumerClient<>(testConfig, consumer);) {
      ...
      Set<KafkaConsumerRecord> consumedRecords = Sets.newHashSet(
          kafka1Client.consume(new KafkaPartition.Builder().withId(0).withTopicName("test_topic").build(), 0l, 100l));
      ...
      Set<Kafka1ConsumerClient.Kafka1ConsumerRecord<String, String>> expected =
          ImmutableSet.of(new Kafka1ConsumerClient.Kafka1ConsumerRecord<>(record0),
              new Kafka1ConsumerClient.Kafka1ConsumerRecord<>(record1),
              new Kafka1ConsumerClient.Kafka1ConsumerRecord<>(record2));
  ...
  @Test
  public void testConsumeWithRetry()
      throws Exception {
    // Set max attemps to poll as 3, with retry interval 1ms
    Config testConfig = ConfigFactory.parseMap(
        ImmutableMap.of(ConfigurationKeys.KAFKA_BROKERS, "test", "source.kafka.poll.max.attempt.count", 3,
            "source.kafka.poll.retry.interval.ms", 2));
    MockConsumer<String, String> consumer = new MockConsumer<String, String>(OffsetResetStrategy.NONE);
    consumer.assign(Arrays.asList(new TopicPartition("test_topic", 0)));
    ...
    // Return no records in initial two poll attempts to trigger retries. Return records only in the 3rd attempt.
